...
class ReplayMemory():
    ...
    def sample(self, batch_size, beta = 0.4):
        ...
        if self.prioritized:
            ...
            random = np.random.choice(size, size = batch_size,
                replace=False
            )
            i = (self.i-1) % self.capacity
            recent = list(range(i,0,-1))[:batch_size]
            batch = list(random) + recent
            ...
            ### full prio
            # i = (self.i-1) % self.capacity
            # batch = list(range(i,0,-1))[:batch_size]
