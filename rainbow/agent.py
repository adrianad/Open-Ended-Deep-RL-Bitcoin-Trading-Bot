...
class Rainbow:
    def __init__(self,
        ...
        self.model.compile(
            optimizer= tf.keras.optimizers.Adam(learning_rate=self.learning_rate, epsilon= 1.5E-4)
        )
    ...
    def log(self, i_env = 0):
        text_print =f"\
↳ Env {i_env} : {self.episode_count[i_env]:03} : {self.steps: 8d}   |   {self.format_time(datetime.datetime.now() - self.start_time)}   |   Epsilon : {self.get_current_epsilon()*100: 4.2f}%"
        print(text_print)
    ...
    def pick_action(self, *args, **kwargs):
        if self.distributional: return self._distributional_pick_action(*args, **kwargs).numpy()[0]
